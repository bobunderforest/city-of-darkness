import { type Vector, vec } from 'utils/Vector'
import initialState from 'components/sections/SectionIntro/initialState'
import { getTransform } from 'vendor/matrixTransform'
import bgMask from 'components/sections/SectionIntro/mask'
...
import { smokemachine } from 'vendor/smoke'

import { noise } from 'vendor/parlin-noise'
  ...
  renderLetters = () => {
    ...
    this.state.textDrawings.forEach((itm, i) => {
      ...
      const textW = measureText(
        itm.text,
        classNames.text + (itm.isChinese ? classNames.textChineese : ''),
        fz,
      )
      ...
      stopBoxes.forEach(
        ({ x1, y1, x2, y2, nx1, ny1, nx2, ny2, p1, np1, p2, np2 }, j) => {
          ...
          perspective.className = classNames.textPerspective
          ...
          if (itm.isChinese) text.className = classNames.textChineese
          ...
          textMover.className = classNames.textMover
